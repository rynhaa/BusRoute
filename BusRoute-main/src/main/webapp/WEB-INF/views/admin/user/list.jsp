	    ...
	    <form action="/admin/user/list" method="get">
	    	<input type="hidden" name="pageNum" value="1">
	      	...
	      	<select name="is_deleted">
	      		...
	      		<option value="0" ${is_deleted == '0' ? 'selected' : ''}>정상회원</option>
	      		<option value="1" ${is_deleted == '1' ? 'selected' : ''}>탈퇴회원</option>
	      	</select>
  		...
  		<table>
  			...
  			<tbody>
  				<c:if test="${empty userList}">
		            <tr>
		              <td colspan="5" style="text-align: center; padding: 20px; color: #777;">사용자 정보가 없습니다.</td>
		            </tr>
	          	</c:if>
	          	<c:forEach items="${userList}" var="users" varStatus="loop">
					<tr>
				    	<%-- <td><input type="checkbox" name="userIds" value="${users.unum}" /></td> --%>
				    	<td>${(page - 1) * pageSize + loop.index + 1}</td>
				    	<td><a href="${pageContext.request.contextPath}/admin/user/detail?unum=${users.unum}">${users.username}</a></td>
				    	<td><a href="${pageContext.request.contextPath}/admin/user/detail?unum=${users.unum}">${users.userid}</a></td>
				    	<td><fmt:formatDate pattern="yyyy-MM-dd" value="${users.create_at}" /></td>
				    	<td><strong>${users.is_deleted ? "<span style='color:red;'>탈퇴</span>" : "정상"}</strong></td>
				  	</tr>
	          	</c:forEach>
  			</tbody>
  		</table>
