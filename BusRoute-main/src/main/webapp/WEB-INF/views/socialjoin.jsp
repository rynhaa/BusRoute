...
<div id="wrapper">
	<h3>회원가입</h3><br>
	<div id="join-form">
		<form action="/socialjoin" method="post" onsubmit="return Join()">
	        <table>
	        	<tr>
	                <td>비밀번호</td>
	            ...
	            <tr>
	                <td>성명</td>
	                <td><input type="text" name="username" id="username" value="${username}" readonly placeholder="성명을 입력해주세요."><br>
	                <label id="namemsg" class="msg" style="font-size:13px"></label></td>
	            </tr>
	            <tr>
	                <td>전화번호</td>
	                <td><input type="text" name="phone" id="phone" value="${phone}" placeholder="(-)포함 전화번호를 입력해주세요."><br>
	                <label id="phonemsg" class="msg" style="font-size:13px"></label></td>
	            </tr>
	            <tr>
	                <td>이메일</td>
	                <td><input type="email" name="email" id="email" value="${email}" readonly placeholder="이메일을 입력해주세요.">
...
function Join(){
	...
	if (!isEmailIdAvailable) {
		emailmsg.innerHTML = "이메일 중복확인이 필요합니다.";
		emailmsg.style.color = "red";
        return false; // 폼 제출 막기
    }
	
	
	if (pwre == true && pwcheckre == true && phonere == true && addressre == true) {
	    return true;
	} else {
	    return false;
	}
